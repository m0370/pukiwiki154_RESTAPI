...
> **⚠ リバースプロキシ・CDN の配下では `--ip` は機能しません**
>
> `--ip` は `$_SERVER['REMOTE_ADDR']` と照合します。Cloudflare 等の CDN や
> リバースプロキシを挟んでいると、これは**プロキシのエッジ IP** になり、
> 実際のクライアント IP ではありません。結果として次のどちらかになります:
>
> - プロキシの IP を許可リストに書く → 実質「誰でも通る」（制限になっていない）
> - 実クライアントの IP を書く → 誰も通らない（常に 403）
>
> 実クライアント IP は `CF-Connecting-IP` や `X-Forwarded-For` に入っていますが、
> **これらのヘッダを無検証で採用してはいけません**。クライアントが自由に詐称でき、
> プロキシを迂回してオリジンへ直接到達できる経路があれば IP 制限を突破されます。
> 正しく対応するには、
>
> 1. `REMOTE_ADDR` が信頼するプロキシの IP レンジに含まれることを検証したうえで
>    ヘッダを採用する（Cloudflare なら公開されている IPv4 15 / IPv6 7 レンジ。変動する）
> 2. 実クライアントが IPv6 のことが多いため、**IPv6 CIDR 照合の実装**もあわせて行う
>    （現状は IPv4 のみ対応で、IPv6 は fail-safe 拒否）
>
> の両方が必要です。現状の実装はどちらも行っていません。
> プロキシ配下では `--ip` を使わず、キーの秘匿・`--expires`・スコープ分離で守ってください。
...
| `--ip` 付きキーが常に 403 / 逆に誰でも通る | リバースプロキシ・CDN 配下では `REMOTE_ADDR` がプロキシの IP になる。プロキシ配下では `--ip` を使わない（[3 節](#3-api-キーの発行と管理)の警告） |
