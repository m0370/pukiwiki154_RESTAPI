...
// =========================================================================
section('bootstrap: DocRoot 内 data/ の露出判定');
// =========================================================================
mkdir($tmp . '/docroot/data', 0755, true);
mkdir($tmp . '/outside-data', 0755, true);
mkdir($tmp . '/docrootdata', 0755, true); // 前方一致の罠検出用（/docroot の部分文字列）

$in_root  = ['DOCUMENT_ROOT' => $tmp . '/docroot'];
ok(rest_data_dir_is_exposed($tmp . '/docroot/data', $in_root, 'cli') === false,
    'CLI（MCP・make-key・テスト）ではガード対象外');
ok(rest_data_dir_is_exposed($tmp . '/docroot/data', $in_root, 'cli-server') === true,
    "php -S（cli-server）は CLI 扱いしない（'cli' 完全一致）");
ok(rest_data_dir_is_exposed($tmp . '/docroot/data', $in_root, 'apache2handler') === true,
    'DocRoot 配下ならサーバー種別に関係なく露出と判定（Apache でも true）');
ok(rest_data_dir_is_exposed($tmp . '/docroot', $in_root, 'fpm-fcgi') === true,
    'DocRoot そのものも露出と判定');
ok(rest_data_dir_is_exposed($tmp . '/outside-data', $in_root, 'fpm-fcgi') === false,
    'DocRoot 外なら非露出');
ok(rest_data_dir_is_exposed($tmp . '/docrootdata', $in_root, 'fpm-fcgi') === false,
    'パス前方一致の誤検出をしない（/docroot と /docrootdata）');
ok(rest_data_dir_is_exposed($tmp . '/docroot/data', [], 'fpm-fcgi') === false,
    'DOCUMENT_ROOT 未設定なら判定しない');
ok(rest_data_dir_is_exposed($tmp . '/no-such-dir', $in_root, 'fpm-fcgi') === false,
    '未存在ディレクトリは判定しない（realpath 不能）');
...
expect_api_error(
    fn() => $REST_PAGES->read(':config/plugin'),
    403, "システムページ（':' 始まり）は直接 read も 403"
);

// 本文サイズ上限（PKWK_REST_MAX_BODY_BYTES で上書き可）
putenv('PKWK_REST_MAX_BODY_BYTES=100');
ok(PageStore::maxBodyBytes() === 100, '本文上限を環境変数で上書きできる');
expect_api_error(
    fn() => $REST_PAGES->write('サイズ上限テスト', str_repeat('a', 101), $EMPTY, 'unit-test'),
    413, '本文上限超は 413'
);
$r_sz = $REST_PAGES->write('サイズ上限テスト', str_repeat('a', 99) . "\n", $EMPTY, 'unit-test');
ok($r_sz['is_new'] === true, '上限ちょうど（100 bytes）は書き込める');
putenv('PKWK_REST_MAX_BODY_BYTES');
ok(PageStore::maxBodyBytes() === PageStore::DEFAULT_MAX_BODY_BYTES, '環境変数を外すと既定値 1MB に戻る');
...
ok($list['total'] === 4, 'listPages が全ページを数える（total=' . $list['total'] . '）');
